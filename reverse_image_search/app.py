...
def main():
    ...
    with col1:
        img_dir = st.text_input("Enter the Absolute directory path where to search:", placeholder='e.g. /home/username/some-folder-name')
    ...
    with col12:
        img_path2 = st.text_input("Absolute filepath of Image you want to compare.", placeholder='e.g. /home/username/picture/hello.jpg')
    
    with col3:
        similar_images = st.number_input("Number of similar images to display:", min_value=-1, max_value=50, value=7, step=1)

    with col4:
        save_model = st.checkbox('Save Model', value=False, help='Save the model for faster loads, check if you search in same folder again and again')
        recursive = st.checkbox('Recursive', value=False, help = 'Search recursively for images in child folders')    
    ...
    if submit_button:
        if os.path.isdir(img_dir) and (img_path or img_path2):
            ...
            image_similarity = ImageSimilarity(img_dir=Path(img_dir), recursive=recursive, save_model=save_model)

            with st.spinner('Wait for it... Model training'):
                similar_image_paths, similarity_value = image_similarity.find_similar_images2(main_image, k=similar_images)
